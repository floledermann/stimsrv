...
The HTML structure established by stimsrv is quite simple. Inside the `body`, a `section` with `id=interface-*X*` is created for each UI component *X* of the current task which is available for the given role. Within those `sections`, each UI component may create the required HTML.
...
This would be a typical HTML structure:
...
To modify the appearance and layout of the stimsrv frontend, you can add a `css` property to the experiment, individual roles and devices and most tasks. These styles will be added to the frontend as appropriate.

By default, stimsrv uses the flexbox layout for laying out the UI elements. By overriding the appropriate CSS properties (potentially only for some roles or devices, or only for some tasks), the layout can be adjusted and even the order of items can be changed using the `order` property. If needed, the layout mechanism can also be changed to grid or any other CSS layout mechanism by overriding the approprate CSS properties.

Here is an example experiment configuration that adjusts the font size on some devices and changes the order of UI components for the first task:
